...
function MyTickets() {
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => { toast.success("RSVP cancelled"); qc.invalidateQueries({ queryKey: ["my-tickets"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  if (!user) return <SiteLayout><div className="p-20 text-center">Please <Link to="/auth" search={{ returnTo: "/tickets" }} className="text-primary underline">sign in</Link>.</div></SiteLayout>;
  ...
  return (
    <SiteLayout>
      <div className="container mx-auto max-w-3xl px-4 py-10">
        ...
        {isLoading ? <p className="mt-6">Loading…</p> : data.length === 0 ? (
          <div className="mt-10 rounded-lg border border-dashed border-border p-12 text-center">
            ...
            <Link to="/explore"><Button className="mt-4">Explore events</Button></Link>
          </div>
        ) : (
                  ...
                  <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start">
                    <div className="flex-1 space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <Link to="/events/$eventId" params={{ eventId: ev.id }} className="font-semibold hover:underline">{ev.title}</Link>
                        <Badge variant={t.status === "going" ? "default" : "secondary"}>
                          {t.status === "going" ? "Going" : `Waitlist${t.waitlist_position ? ` #${t.waitlist_position}` : ""}`}
                        </Badge>
                      </div>
                      {host && <p className="text-sm text-muted-foreground">by {host.name}</p>}
                      <p className="flex items-center gap-2 text-sm text-muted-foreground"><Calendar className="h-4 w-4" />{formatDateTime(ev.start_at, ev.timezone)} ({ev.timezone})</p>
                      <p className="flex items-center gap-2 text-sm text-muted-foreground">
                        {ev.online_link ? <Video className="h-4 w-4" /> : <MapPin className="h-4 w-4" />}
                        {ev.online_link || ev.venue_address}
                      </p>
                      {t.status === "waitlist" && (
                        <p className="text-sm text-amber-600 dark:text-amber-400">You are on the waitlist. We'll promote you automatically if a spot opens.</p>
                      )}
                    </div>
                    <TicketActions
                      rsvp={t}
