  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: invite } = await supabase
        ...
        .maybeSingle();
      if (!invite) {
        if (!cancelled) setStatus({ kind: "invalid", message: "Invalid invite link." });
        return;
      }
      if (invite.expires_at && new Date(invite.expires_at) < new Date()) {
        if (!cancelled) setStatus({ kind: "invalid", message: "This invite has expired." });
        return;
      }
      const hostName =
        (invite as unknown as { hosts: { name: string } }).hosts?.name ?? "this host";
