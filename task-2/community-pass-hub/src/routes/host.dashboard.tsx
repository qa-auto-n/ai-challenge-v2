...
function HostDashboard() {
  ...
  useEffect(() => {
    if (!selectedHostId && hostMemberships.length > 0) setSelectedHostId(hostMemberships[0].host_id);
  }, [hostMemberships, selectedHostId]);

  const { data: hostInfo } = useQuery({
    queryKey: ["host-info", selectedHostId],
    enabled: !!selectedHostId,
    queryFn: async () => (await supabase.from("hosts").select("*").eq("id", selectedHostId).maybeSingle()).data,
  });
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<EventWithCounts[]> => {
      const { data: events } = await supabase.from("events").select("*").eq("host_id", selectedHostId).order("start_at", { ascending: false });
      const evs = events ?? [];
      return Promise.all(evs.map(async (e) => {
        const [g, w, c] = await Promise.all([
          ...
          supabase.from("rsvps").select("id", { count: "exact", head: true }).eq("event_id", e.id).eq("status", "going").not("checked_in_at", "is", null),
        ]);
  ...
  if (!user) return <SiteLayout><div className="p-20 text-center">Please <Link to="/auth" search={{ returnTo: "/host/dashboard" }} className="text-primary underline">sign in</Link>.</div></SiteLayout>;
  ...
  if (hostMemberships.length === 0) {
    return <NoAccess message="You need a Host role to access the dashboard. Become a host or use a host invite link." />;
  }
      ...
      <div className="container mx-auto px-4 py-10 space-y-8">
        ...
        {hostMemberships.length > 1 && (
          <div className="max-w-xs">
            ...
            <select id="host-select" className="flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
              value={selectedHostId} onChange={(e) => setSelectedHostId(e.target.value)}>
              {hostMemberships.map((m) => (
                <option key={m.host_id} value={m.host_id}>{m.hosts?.name}</option>
              ))}
            </select>
          </div>
        )}
        ...
        {hostInfo && (
          <Card>
            <CardContent className="flex flex-wrap items-center gap-4 p-4">
              {hostInfo.logo_url && <img src={hostInfo.logo_url} alt={hostInfo.name} className="h-14 w-14 rounded-full object-cover" />}
              <div>
        ...
        {isLoading ? <p className="text-muted-foreground">Loading…</p> : (
          <Tabs defaultValue="upcoming">
            ...
            <TabsContent value="upcoming"><EventTable list={upcoming} /></TabsContent>
            <TabsContent value="past"><EventTable list={past} /></TabsContent>
          </Tabs>
        )}
...
function EventTable({ list }: { list: EventWithCounts[] }) {
  if (list.length === 0) return <Card className="mt-4"><CardContent className="p-12 text-center text-muted-foreground">No events here yet.</CardContent></Card>;
  return (
    <Card className="mt-4">
            ...
            {list.map((e) => (
              <TableRow key={e.id}>
                <TableCell className="font-medium">
                  <div>{e.title}</div>
                  <div className="text-xs text-muted-foreground">{e.online_link ? "Online" : (e.venue_address ?? "—")}</div>
                </TableCell>
                ...
                <TableCell><Badge variant="outline">{e.visibility}</Badge></TableCell>
                <TableCell className="text-right">{e.going}/{e.capacity}</TableCell>
                <TableCell className="text-right">{e.waitlist}</TableCell>
                <TableCell className="text-right">{e.checked}</TableCell>
                <TableCell>
                  <div className="flex flex-wrap justify-end gap-2">
                    <Link to="/host/events/$eventId/edit" params={{ eventId: e.id }}><Button variant="outline" size="sm">Manage</Button></Link>
                    <EventActions event={e} />
                    <Link to="/check-in/$eventId" params={{ eventId: e.id }}><Button variant="outline" size="sm">Check-in</Button></Link>
                    <CsvExportButton label="RSVPs CSV" eventId={e.id} eventTitle={e.title} variant="rsvps" />
                    <CsvExportButton label="Attendance CSV" eventId={e.id} eventTitle={e.title} variant="attendance" />
                  </div>
                </TableCell>
              </TableRow>
            ))}
