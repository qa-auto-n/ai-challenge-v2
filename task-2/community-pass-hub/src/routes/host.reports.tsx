...
function Reports() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: async (): Promise<EnrichedReport[]> => {
      ...
      const [{ data: events }, { data: photos }, { data: profiles }] = await Promise.all([
        ...
        photoIds.length
          ? supabase.from("gallery_photos").select("id, image_url, event_id").in("id", photoIds)
          : Promise.resolve({ data: [] }),
      ...
      return list.map((r) => {
        ...
        if (r.target_type === "event") {
          return {
            ...r,
            event_title: evMap.get(r.target_id)?.title,
  ...
  const hide = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["open-reports"] });
    },
    ...
    <SiteLayout>
      <div className="container mx-auto px-4 py-10">
        ...
        <p className="mt-1 text-muted-foreground">
          Open reports for events and photos under your hosts.
        </p>
        {data.length === 0 ? (
              ...
              <Card key={r.id}>
                <CardContent className="flex flex-wrap items-start justify-between gap-4 p-4">
                  <div className="flex flex-1 items-start gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {r.reason || <em>No reason given</em>}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Reported by {r.reporter_name || r.reporter_email || "anonymous"} ·{" "}
                  ...
                  <div className="flex gap-2">
                    {r.target_type === "event" && (
                      <Link to="/events/$eventId" params={{ eventId: r.target_id }}>
