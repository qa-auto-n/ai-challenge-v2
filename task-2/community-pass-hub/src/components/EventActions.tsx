...
export function EventActions({ event, size = "sm" }: Props) {
  ...
  return (
    <>
      {event.status === "draft" ? (
        <Button variant="outline" size={size} onClick={() => setStatus.mutate("published")} disabled={setStatus.isPending}>
          <Eye className="h-4 w-4" /> Publish
        </Button>
      ) : (
        <Button variant="outline" size={size} onClick={() => setStatus.mutate("draft")} disabled={setStatus.isPending}>
          <EyeOff className="h-4 w-4" /> Unpublish
        </Button>
      )}
      <Button variant="outline" size={size} onClick={() => duplicate.mutate()} disabled={duplicate.isPending}>
